...
# It is a one hot encoder function, because keras do not work with 
# just the idea of indexes(overfit). 
def y2indicator(Y):
  N = len(Y)
  K = len(set(Y))
  I = np.zeros((N, K))
...
data = pd.read_csv('fashionmnist_img/fashion-mnist_train.csv')
data = data.values
np.random.shuffle(data) 
										# normalize the data/pixels by dividing by 255
X = data[:, 1:].reshape(-1, 28, 28, 1) / 255.0 # img must be in height x width x color. 
